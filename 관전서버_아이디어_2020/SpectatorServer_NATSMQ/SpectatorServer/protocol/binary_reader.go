...
func (p *RawPacketData) ReadU8() (ret uint8, err error) {
	_ret, _err := p.ReadByte()
	ret = uint8(_ret)
	err = _err
	return
...
func (p *RawPacketData) ReadString() (bytes []byte, err error) {
	if p.pos+2 > len(p.data) {
	...
	size, _ := p.ReadU16()
	if size <= 0 || p.pos+int(size) > len(p.data) {
		err = errors.New("read string TotalData failed")
		return
	}
...
func (p *RawPacketData) Seek(seek int) {
	p.pos = seek
	return
}
/*
...
func (p *RawPacketData) WriteU8(v uint8) {
	p.data[p.pos] = (byte)(v)
	p.pos++
}
...
func (p *RawPacketData) WriteLenAndBytes(v []byte) {
	byteLen := len(v)

	p.WriteU16(uint16(byteLen))
...
func (p *RawPacketData) WriteZeros(n int) {
	for i := 0; i < n; i++ {
		p.TotalData = append(p.TotalData, byte(0))
	}
}

func (p *RawPacketData) WriteBool(v bool) {
	if v {
		p.TotalData = append(p.TotalData, byte(1))
	} else {
		p.TotalData = append(p.TotalData, byte(0))
	}
}
...
func (p *RawPacketData) WriteByte(v byte) {
	p.TotalData = append(p.TotalData, v)
}
...
func (p *RawPacketData) WriteString(v string) {
	...
	p.TotalData = append(p.TotalData, bytes...)
}

func (p *RawPacketData) WriteU24(v uint32) {
	p.TotalData = append(p.TotalData, byte(v>>16), byte(v>>8), byte(v))
}
