...
public:
	...
	// lostrate: 주당 왕복 패킷 손실률, 기본값 10%
	// rttmin：RTT 최소값, 기본값 60
	// rttmax：RTT 최대값, 기본값 125 
	LatencySimulator(int lostrate = 10, int rttmin = 60, int rttmax = 125, int nmax = 1000): 
		r12(100), r21(100) {
		current = iclock();		
		this->lostrate = lostrate / 2;	// 위의 수치는 편도 기준 왕복 패킷 손실률을 2로 나눈 값이다.
	...
	// 데이터 전송
	// peer - 엔드포인트 0/1, 0에서 송신, 1에서 수신; 1에서 송신, 0에서 수신 
