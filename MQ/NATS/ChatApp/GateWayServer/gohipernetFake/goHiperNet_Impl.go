...
func _start_TCPServer_block(config *NetworkConfig, networkFunctor SessionNetworkFunctors) {
	...
	for {
		...
		client := &TcpSession{
			SeqIndex:       SeqNumIncrement(serverIndex),
			TcpConn:        conn,
...
func _stop() {
	Logger.Info("Stop Network - Start")

	_mClientListener.Close()
	//TODO _tcpSessionManager에서 관리 중인 세션의 연결을 다 끊고, 고루틴 중단까지 기다린다.
	Logger.Info("Stop Network - End")
}
...
func SeqNumIncrement(serverIndex uint16) uint64 {
	newValue := atomic.AddUint64(&_seqNumber, 1)
	newValue *= 1000
	newValue += uint64(serverIndex)
	return newValue
}
